...
/***/ "./resources/laravel/js/ccmMonthlyMonitoring.js":
/*!******************************************************!*\
  !*** ./resources/laravel/js/ccmMonthlyMonitoring.js ***!
  \******************************************************/
/*! no static exports found */
...
var URL_POPULATE_HIPPA = "/ccm/ajax/populate_hippa";
var URL_POPULATE_HOMESERVICE = "/ccm/ajax/populate_home_service";
var URL_POPULATE_CALL_CLOSE = "/ccm/ajax/populate_call_close";
var URL_POPULATE_CALL_WRAPUP = "/ccm/ajax/populate_call_wrapup";
var URL_POPULATE_FOLLOWUP = "/ccm/ajax/populate_follow_up";
var URL_SAVE = "/ajax/tcm_save";
var URL_SUBMIT = "/ajax/tcm_submit";
var URL_PRINT = "/tcm/printing/";
var URL_POPULATE_DIAGNOSIS_LIST = '/ccm/patient-care-plan-list';
...
var onPreparationFollowUp = function onPreparationFollowUp(formObj, fields, response) {
  if (response.status == 200) {
    ...
    if (module == 'rpm') {
      setTimeout(function () {
        util.totalTimeSpent(patient_id, module_id, stage_id);
        $('.alert').fadeOut('fast');
      }, 5000);
...
var onPreparationResearchFollowUp = function onPreparationResearchFollowUp(formObj, fields, response) {
  if (response.status == 200) {
    util.updateTimer($("input[name='patient_id']").val(), 1, $("input[name='module_id']").val());
    $(".form_start_time").val(response.data.form_start_time);
    var patient_id = $("#hidden_id").val();
...
var onRelationship = function onRelationship(formObj, fields, response) {
  if (response.status == 200) {
    util.updateTimer($("input[name='patient_id']").val(), 1, $("input[name='module_id']").val());
    $(".form_start_time").val(response.data.form_start_time);
...
var onFollowUpForm = function onFollowUpForm(formObj, fields, response) {
...
var onFollowUpFormEditData = function onFollowUpFormEditData(formObj, fields, response) {
  if (response.status == 200) {
    util.updateTimer($("input[name='patient_id']").val(), 1, $("input[name='module_id']").val());
    util.totalTimeSpentByCM();
    $(".form_start_time").val(response.data.form_start_time);
...
var onText = function onText(formObj, fields, response) {
  if (response.status == 200) {
    ...
    if (str.includes('HTTP') || response.status == 406) {
      var errormsg = '<div class="alert alert-danger alert-block " style="margin-left: 1.1em;margin-right: 1.1em;"><button type="button" class="close" data-dismiss="alert">�</button><strong>' + response.data.msg + '</strong></div>';
    ...
    var table = $('#callwrap-list');
    table.DataTable().ajax.reload();
    util.getToDoListData($("input[name='patient_id']").val(), $("input[name='module_id']").val()); //util.getDataCalender($("input[name='patient_id']").val(), $("input[name='module_id']").val());
...
var callWrapUpShowHide = function callWrapUpShowHide() {
  ...
  } else {// alert('newcheckbox10  not checked');
    // $("form[name='callwrapup_form'] #authorizeddiv").show();  
    // $("form[name='callwrapup_form'] #veteransdiv").show();   
    // $("form[name='callwrapup_form'] #resourcediv").show();   
    // $("form[name='callwrapup_form'] #maileddiv").show();
...
var init = function init() {
  ...
  util.updateTimeEveryMinutes(patient_id, module_id, start_time); // util.redirectToWorklistPage();
  ...
  callWrapUpShowHide(); // starts here
  ...
  util.getPatientStatus(patient_id, module_id);
  util.gatCaretoolData(patient_id, module_id);
  util.getToDoListData($("#patient_id").val(), module_id); // util.getToDoListCalendarData($("#patient_id").val(), module_id);
  //util.getDataCalender($("#patient_id").val(), module_id);

  ccmcpdcommonJS.getFollowupList($('#patient_id').val(), module_id); // Research followup data presentEMR
  ...
  var data = "";
  var preparationNotesFormPopulateURL = URL_POPULATE_PREPARATION_NOTES + "/" + patientId + "/current";
  ccmMonthlyMonitoring.populateForm(patientId, preparationNotesFormPopulateURL); // ends here
  ...
  $("form[name='callwrapup_form'] #no_additional_services_provided").change(function () {
    if ($("form[name='callwrapup_form'] #no_additional_services_provided").is(":checked")) {
      ...
      $("form[name='callwrapup_form'] #veterans_services").prop("checked", false);
      $("form[name='callwrapup_form'] #authorized_cm_only").prop("checked", false);
    } else {// alert('authorized not checked');
      // $("form[name='callwrapup_form'] #authorizeddiv").show();  
      // $("form[name='callwrapup_form'] #veteransdiv").show();   
      // $("form[name='callwrapup_form'] #resourcediv").show();   
      // $("form[name='callwrapup_form'] #maileddiv").show();
  ...
  form.ajaxForm("call_preparation_preparation_followup_form", onPreparationFollowUp, function () {
    ...
    if ((checked1 == true || checked2 == true || checked3 == true || checked4 == true) && (reportchecked1 == true || reportchecked2 == true || reportchecked3 == true || reportchecked4 == true || reportchecked5 == true)) {
      ...
      if ($("form[name='call_preparation_preparation_followup_form'] .form_start_time").val() == "undefined" || $("form[name='call_preparation_preparation_followup_form'] .form_start_time").val() == '') {
        var form_start_time = $("#page_landing_times").val();
        $("form[name='call_preparation_preparation_followup_form'] .form_start_time").val(form_start_time);
      }
      ...
      setTimeout(function () {
        $('form[name="call_preparation_preparation_followup_form"]').find(":submit").attr("disabled", false);
      }, 3000);
  ...
  form.ajaxForm("research_follow_up_preparation_followup_form", onPreparationResearchFollowUp, function () {
    ...
    if ((checked1 == true || checked2 == true || checked3 == true || checked4 == true) && (reportchecked1 == true || reportchecked2 == true || reportchecked3 == true || reportchecked4 == true || reportchecked5 == true)) {
      ...
      setTimeout(function () {
        $('form[name="research_follow_up_preparation_followup_form"]').find(":submit").attr("disabled", false);
      }, 3000);
  ...
  form.ajaxForm("callwrapup_form", ccmcpdcommonJS.onCallWrapUp, function () {
    ...
    var checkbox9_length = $('#authorizeddiv input:checked').length; // if(jQuery('#frmTest input[type=checkbox]:checked').length) { … }
    // var q = (jQuery('#routine_response input[type=checkbox]:checked').length) ;

    if (checkbox_1 == true && checkbox1_length > 0 || checkbox_2 == true && checkbox2_length > 0 || checkbox_3 == true && checkbox3_length > 0 || checkbox_4 == true && checkbox4_length > 0 || checkbox_5 == true && checkbox5_length > 0 || checkbox_6 == true && checkbox6_length > 0 || checkbox_7 == true && checkbox7_length > 0 || checkbox_8 == true && checkbox8_length > 0 || checkbox_9 == true && checkbox9_length > 0 || checkbox_10 == true) {
      ...
      setTimeout(function () {
        $('form[name="callwrapup_form"]').find(":submit").attr("disabled", false);
      }, 3000);
      return false;
    }
  });
  ...
  $('#call_preparation_draft').on('click', function () {
    ...
    $.ajax({
      ...
      success: function success(data) {
        ...
        var timer_paused = $("form[name='call_preparation_preparation_followup_form'] input[name='end_time']").val();
        $("#timer_start").val(timer_paused);
        $(".form_start_time").val(data.form_start_time);
      }
    });
  ...
  $("input[name='call_status']").click(function () {
    ...
    if (checked_call_option == "1") {
      ...
      $("#call_scripts_select option:last").attr("selected", "selected").change();
...
/***/ 3:
/*!************************************************************!*\
  !*** multi ./resources/laravel/js/ccmMonthlyMonitoring.js ***!
  \************************************************************/
/*! no static exports found */
/***/ (function(module, exports, __webpack_require__) {

module.exports = __webpack_require__(/*! /var/www/html/mnt1/rcaregit/resources/laravel/js/ccmMonthlyMonitoring.js */"./resources/laravel/js/ccmMonthlyMonitoring.js");
