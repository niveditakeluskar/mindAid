...
//created by radha
 ...
 {    
  ...
  //             not in (select distinct observation_id from rpm.patient_cons_observations) offset 0 limit 10"));
   ...
   from api.webhook_observation where fetch_status = 0  " ));  
   ...
   foreach($getdata as $g)
   {
    ...
    if($patientid!=null && $mrn!= null ){
     ...
     if((!$checkpatientservicesinccm->isEmpty()) && ($patientservices->isEmpty()) ){
           ...
           'mrn'=>$mrn,'patient_id'=>$patientid,'observation_id'=>$observationid, 'device_code'=>$deviceid);
           $exception = ApiException::create($a);
     }
    ...
    //  else{
      ...
      if($time > $suspended_from  &&  $time < $suspended_to &&  $patientstatus== 2 ){
         ...
         $a = array('api'=>(Config::get('global.authurl')).'observations/{id}',
         'parameter'=>$parameter,'exception_type'=>'alert','incident'=>'deactive','webhook_id'=>$id,        
         'mrn'=>$mrn,'patient_id'=>$patientid,'observation_id'=>$observationid, 'device_code'=>$devdiceid);
         $exception = ApiException::create($a);
      }
        ...
        $exception = ApiException::create($a);
                ...
                if (array_key_exists('message', $newgetdata)){
                  ...
                  ApiErrorLog::create($a);
				  $up = WebhookObservation::where('id',$id)->update(['fetch_status'=>4]);	//for no observation found 11th jan 2023	
                ...
                //  dd($newgetdata);
                  ...
                  $insertdata=array(
                    ...
                    $exception = ApiException::create($a); 
                      ...
                      $this->saveRPMReadings($idarrays,$ob,$patientid,$deviceid,$mrn,$observationid);   
                    }
 ...
 public function saveRPMReadings($idarrays,$obs,$patientid,$deviceid,$mrn_no,$observationid)
 {
  ...
  $checkexceptions = ApiException::where('patient_id',$patientid)                 
                    ...
                    if(count($checkexceptions)>0){
                      ...
                      WebhookObservation::where('id',$checkexceptions[0]->webhook_id)->update(['fetch_status'=>0]); 
...
//  ----------------------------threshold and  alert status code------------------------------------------------------------------------------ 
  ...
  //             if(empty($getdata)){
  //               $msg = 'Empty observation record';  
  //                           $p = explode(" ",$msg);
  //                           $parameter = $p[0]."-".$p[1]."-".$p[2]; 
  //                           $a = array('api'=>(Config::get('global.authurl')).'observations/{id}',
  //                           'parameter'=>$parameter,'exception_type'=>'alert','incident'=>'emptyobservation','webhook_id'=>$id,        
  ...
  //                     $msg = 'Observation id null';  
  //                           $p = explode(" ",$msg);
  //                           $parameter = $p[0]."-".$p[1]."-".$p[2]; 
  //                           $a = array('api'=>(Config::get('global.authurl')).'observations/{id}',
  //                           'parameter'=>$parameter,'exception_type'=>'alert','incident'=>'emptyobservationid','webhook_id'=>$id,        
  //                           'mrn'=>null,'patient_id'=>null,'observation_id'=>null, 'device_code'=>null);
  //                           $exception = ApiException::create($a);
  ...
  //                     $parameter = $p[0]."-".$p[1]."-".$p[2]; 
  //                     $a = array('api'=>(Config::get('global.authurl')).'observations/{id}',
  //                     'parameter'=>$parameter,'exception_type'=>'alert','incident'=>'emptyxmitid','webhook_id'=>$id,        
  //                     'mrn'=>null,'patient_id'=>null,'observation_id'=>null, 'device_code'=>null);
  //                     $exception = ApiException::create($a);
  ...
  //                   $msg = 'Xmit id key null';  
  //                   $p = explode(" ",$msg);
  //                   $parameter = $p[0]."-".$p[1]."-".$p[2]."-".$p[3]; 
  //                   $a = array('api'=>(Config::get('global.authurl')).'observations/{id}',
  //                   'parameter'=>$parameter,'exception_type'=>'alert','incident'=>'emptyxmitidkey','webhook_id'=>$id,        
  //                   'mrn'=>null,'patient_id'=>null,'observation_id'=>null, 'device_code'=>null);
  //                   $exception = ApiException::create($a);
  ...
  //                   if($patientservices->isEmpty())
  //                   {
  //                     $msg = 'Patient not enrolled in any services';  
  //                     $p = explode(" ",$msg);
  //                     $parameter = $p[0]."-".$p[1]."-".$p[2]."-".$p[3]."-".$p[4]."-".$p[5]; 
  //                     $a = array('api'=>(Config::get('global.authurl')).'observations/{id}',
  //                     'parameter'=>$parameter,'exception_type'=>'alert','incident'=>'nonenrolled','webhook_id'=>$id,        
  //                     'mrn'=>$mrn_no,'patient_id'=>$patientid,'observation_id'=>$observationid, 'device_code'=>$deviceid);
  //                     $exception = ApiException::create($a);
  //                   }
  //                   else{
  ...
  //                   } //    if($patientid!=null && $mrn_no!= null ){ iska colsing bracket
...
--        p.fname,p.lname,p.email,ob.effdatetime
    ...
    //created and modified by ashvini
 public function saveRPMReadingsNew($idarrays,$obs,$patientid,$deviceid,$mrn_no,$observationid)
 {
   ...
   switch($id){   
     ...
     if($a==null || $a== ''){
      ...
      $msg = 'Empty Spirometer ValueQuantity';  
      $p = explode(" ",$msg);
      $parameter = $p[0]."-".$p[1]."-".$p[2];
      $a = array('api'=>(Config::get('global.authurl')).'observations/{id}',
      'parameter'=>$parameter,'exception_type'=>'alert','incident'=>'missing','webhook_id'=>$id,
    ...
    if (array_key_exists("unit",$a)){ 
      ...
      $msg = 'Spirometer unit of ValueQuantity is null';  
      $p = explode(" ",$msg);
      $parameter = $p[0]."-".$p[1]."-".$p[2];
      $a = array('api'=>(Config::get('global.authurl')).'observations/{id}',
      'parameter'=>$parameter,'exception_type'=>'info','incident'=>'missing','webhook_id'=>$id,
      'mrn'=>$mrn_no,'patient_id'=>$patientid,'observation_id'=>$observationid, 'device_code'=>$deviceid );
      $exception = ApiException::create($a);
...
//-------------------------------------------------------------------------threshold and alert status together---------------------------------------------------------------------------------
      ...
      $msg = 'Spirometer fevValue of ValueQuantity is null';     
      $p = explode(" ",$msg);
      $parameter = $msg;
      $a = array('api'=>(Config::get('global.authurl')).'observations/{id}',
      'parameter'=>$parameter,'exception_type'=>'alert','incident'=>'missing','webhook_id'=>$id,
    ...
    $msg = 'Spirometer Value of ValueQuantity is null';  
    $p = explode(" ",$msg);
    $parameter = $msg;
    $a = array('api'=>(Config::get('global.authurl')).'observations/{id}',
    'parameter'=>$parameter,'exception_type'=>'alert','incident'=>'missing','webhook_id'=>$id,
...
//--------------------------------------------------------------------threshold and alert status together range end------------------------------------------------------------------
     ...
     if($a==null || $a== ''){
      ...
      $msg = 'Empty Glucose ValueQuantity';  
      $p = explode(" ",$msg);
      $parameter = $p[0]."-".$p[1]."-".$p[2];
      $a = array('api'=>(Config::get('global.authurl')).'observations/{id}',
      'parameter'=>$parameter,'exception_type'=>'alert','incident'=>'missing','webhook_id'=>$id,
      ...
      if (array_key_exists("unit",$a)){ 
        ...
        $msg = 'Glucose unit of ValueQuantity is null';  
        $p = explode(" ",$msg);
        $parameter = $p[0]."-".$p[1]."-".$p[2];
        $a = array('api'=>(Config::get('global.authurl')).'observations/{id}',
        'parameter'=>$parameter,'exception_type'=>'info','incident'=>'missing','webhook_id'=>$id,
  ...
  //  ----------------------------  threshold and alert status code------------------------------------------------------------------------------
        ...
        if($checkpatientthreshold->isEmpty())
        {
          ...
          $msg = 'Glucose Value of ValueQuantity is null';     
          $p = explode(" ",$msg);
          $parameter = $msg;
          $a = array('api'=>(Config::get('global.authurl')).'observations/{id}',
          'parameter'=>$parameter,'exception_type'=>'alert','incident'=>'missing','webhook_id'=>$id,
          ...
          // throw new Exception('Glucose Value of ValueQuantity is null or blank alert');
          break;
        }
          ...
          // throw new Exception('Glucose ValueQuantity key missing alert');
        }
...
//  ----------------------------  threshold and alert status code end------------------------------------------------------------------------------  
     ...
     case 'weight-scale' : 
      ...
      if($a==null || $a== ''){
       ...
       $msg = 'Empty WeightScale ValueQuantity';  
       $p = explode(" ",$msg);
       $parameter = $p[0]."-".$p[1]."-".$p[2];
       $a = array('api'=>(Config::get('global.authurl')).'observations/{id}',
       'parameter'=>$parameter,'exception_type'=>'alert','incident'=>'missing','webhook_id'=>$id,
       ...
       if (array_key_exists("value",$a)){ 
         $value = $a->value; 
           ...
           if (array_key_exists("unit",$a)){ 
             ...
             $msg = 'WeightScale unit of ValueQuantity is null';  
             $p = explode(" ",$msg);
             $parameter = $p[0]."-".$p[1]."-".$p[2];
             $a = array('api'=>(Config::get('global.authurl')).'observations/{id}',
             'parameter'=>$parameter,'exception_type'=>'info','incident'=>'missing','webhook_id'=>$id,
...
//--------------------------------------------------------------------threshold and alert status range end------------------------------------------------------------------
         ...
         $msg = 'WeightScale Value of ValueQuantity is null';     
         $p = explode(" ",$msg);
         $parameter = $msg;
         $a = array('api'=>(Config::get('global.authurl')).'observations/{id}',
         'parameter'=>$parameter,'exception_type'=>'alert','incident'=>'missing','webhook_id'=>$id,
...
//  ----------------------------  alert status code------------------------------------------------------------------------------            
              ...
              $msg = 'BloodpressureSystolic Value of ValueQuantity is null';     
              $p = explode(" ",$msg);
              $parameter = $msg;
              $a = array('api'=>(Config::get('global.authurl')).'observations/{id}',
              'parameter'=>$parameter,'exception_type'=>'alert','incident'=>'missing','webhook_id'=>$id,
...
//  ----------------------------  alert status code------------------------------------------------------------------------------  
  ...
  $msg = 'BloodpressureDiastolic Value of ValueQuantity is null';     
  $p = explode(" ",$msg);
  $parameter = $msg;
  $a = array('api'=>(Config::get('global.authurl')).'observations/{id}',
  'parameter'=>$parameter,'exception_type'=>'alert','incident'=>'missing','webhook_id'=>$id,
         ...
         if($dis == 'Hemoglobin saturation with oxygen'){  
          if (array_key_exists("valueQuantity",$obs)){ 
           ...
           if($v==null || $v== ''){
            ...
            $msg = 'Empty Saturation ValueQuantity';  
            $p = explode(" ",$msg);
            $parameter = $p[0]."-".$p[1]."-".$p[2];
            $a = array('api'=>(Config::get('global.authurl')).'observations/{id}',
            'parameter'=>$parameter,'exception_type'=>'alert','incident'=>'missing','webhook_id'=>$id,
...
//  ----------------------------  alert status code------------------------------------------------------------------------------ 
         ...
         // throw new Exception('Spirometer Value of ValueQuantity is null or blank alert');
         break;
       }
          ...
          // throw new Exception('Spirometer ValueQuantity key missing alert');
        }
       ...
       if($dis == 'Resting heart rate')
       {
          ...
          if (array_key_exists("valueQuantity",$obs)){ 
           ...
           if($v==null || $v== ''){
            ...
            $msg = 'Empty Heartrte ValueQuantity';  
            $p = explode(" ",$msg);
            $parameter = $p[0]."-".$p[1]."-".$p[2];
            $a = array('api'=>(Config::get('global.authurl')).'observations/{id}',
            'parameter'=>$parameter,'exception_type'=>'alert','incident'=>'missing','webhook_id'=>$id,
          ...
          if (array_key_exists("value",$v)){ 
           ...
           if (array_key_exists("unit",$v)){ 
            ...
            $msg = 'Heartrate unit of ValueQuantity is null';  
            $p = explode(" ",$msg);
            $parameter = $p[0]."-".$p[1]."-".$p[2];
            $a = array('api'=>(Config::get('global.authurl')).'observations/{id}',
            'parameter'=>$parameter,'exception_type'=>'info','incident'=>'missing','webhook_id'=>$id,
...
//  ----------------------------threshold and  alert status code------------------------------------------------------------------------------ 
            ...
            $msg = 'Heartrate Value of ValueQuantity is null';     
            $p = explode(" ",$msg);
            $parameter = $msg;
            $a = array('api'=>(Config::get('global.authurl')).'observations/{id}',
            'parameter'=>$parameter,'exception_type'=>'alert','incident'=>'missing','webhook_id'=>$id,
        ...
        throw new Exception('Heartrate ValueQuantity key missing alert');
      }
