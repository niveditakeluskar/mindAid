...
{    
  //all functions are cleaned by ashvini (15dec2020) 
    ...
    public function fetchSystemThreshold(Request $request) {
        ...
        $org_threshold_exist = Practices::where('practice_group',$practice_id)->orderby('id','desc')->limit(1)->first();
         }else{
         $practice_threshold_exist ="";	
         $org_threshold_exist ="";
        }
  ...
  // dd($query);
        ...
        ->get();
            ...
            foreach($careManager as $key=>$value){ 
                if($value->id == $row->uid){ $select = 'selected';}else{ $select = '';}         
    ...
    public function patientRegistration(PatientAddRequest $request) {
        ...
        try {
            ...
            if($request->provider_id =='0'){
                $insert_provider_id = array('name' =>$pr,
                                        'practice_id' =>$prac,
                                        'provider_type_id' =>1,
                                        'created_by' =>session()->get('userid')
                                        ); 
   ...
   // print_r($patient_data);
        ...
        if($check_data == false){
            $check_data =Patients::where('id',$request->patient_id)->select('fname','lname','mname','dob')->first();
                    ...
                    $new_record = array('patient_id' => $request->patient_id,
        ...
        if($request->provider_id =='0'){
            $insert_provider_id = array('name' =>$name,
                                       'practice_id' =>$prac,
                                       'provider_type_id' =>1,
                                       'created_by' =>session()->get('userid')
                                      ); 
