...
class CarePlanDevelopmentController extends Controller
{
    ...
    public function diagnosis_code_list($id) {
        ...
        foreach($data as $d){
            ...
            // $p = View_Patient_Diagnosis::where()
			
			$value = date('Y-m-d H:i:s', strtotime($p->review_date));

            dd($value->diffInMonths(\Carbon\Carbon::now()));
            if(isset($p)){
    ...
    public function fetchCarePlanDevelopmentPatientDetails(Request $request) {
        ...
        return  view('Ccm::care-plan-development.patient-details',
                ['patient'=>$patient],
                compact(
                    'last_time_spend',
                    'patient_enroll_date',
    ...
    // try {
        ...
        if(!empty($code)){ 
            ...
            // dd($hidden_id,  $check_exist_code , $check_exist_diagnosis_data, Carbon::today()->toDateString() );                           
            ...
            // dd($check);  
                ...
                if($check== true){
                    ...
                    // dd($result_array_1,$result_array_2,$result_array_3);
    ...
    public function savePatientvitalData(PatientsVitalsDataAddRequest $request) {
        ...
        try {
            ...
            if(( $cv1 || $cv2 || $cv3 || $cv4 || $cv5 || $cv6 || $cv7 || $cv8)=='true'){ 
                ...
                $arraynama = implode(" ",[$cw_height,$cw_weight,$cw_bmi,$cw_blood_pressure,$cw_o2,$cw_pulse_rate,$cw_oxygen]); 
    ...
    public function finalizeCpd(Request $request){
        ...
        if($finalize == 0 && sanitizeVariable($request->billabel) == 0){
            return 0;
        }else{
            return 1;
        }
