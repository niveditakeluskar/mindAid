     ...
     <div class="form-group col-md-5" id="sub">
        <div class="row">
            ...
            <label class="radio radio-primary col-md-4">
                <input type="radio"   name="questionnaire['{{str_replace(' ','_', trim($value->questionTitle)) }}']"  value="{{$labels}}" formControlName="radio" onchange="chabgeStepSix('<?php echo $labels; ?>','<?php echo 'questionnaire'.$num; ?>');" <?php if($key_exist == 1 && $submited_ans[$nq1] == $labels) { echo "checked";}?>> 
                <span>{{$labels}}</span>
