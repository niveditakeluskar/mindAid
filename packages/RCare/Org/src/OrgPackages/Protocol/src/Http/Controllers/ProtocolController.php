...
namespace RCare\Org\OrgPackages\Protocol\src\Http\Controllers;
...
// use RCare\RCareAdmin\AdminPackages\Protocol\src\Http\Requests\RPMVitalsProtocolRequest;
use RCare\Org\OrgPackages\Protocol\src\Http\Requests\RPMVitalsProtocolRequest;  
...
class ProtocolController extends Controller {
        ...
        if( $deviceid!='')
        {
          ...
          if(isset($_FILES) && !empty($_FILES)) {
            ...
            {                  
                ...
                $finalpath="/Vitals-Alert-Protocol-Documentation/".$devicefolder."/".$new_name;
    ...
    {
       if ($request->ajax()) {
        $data = RPMProtocol::with('devices')->with('users')->orderBy('status', 'desc')->orderBy('created_at', 'desc')->get();    
        ...
        ->addColumn('protocol', function($row){
               ...
               $devicenm = str_replace(' ', '-', $row->devices['device_name']);
               if($row->file_name=='')
               {
                $btn ='<a href="#" target="_blank"></a>';
               }
               else
               {
                $btn ='<a href="'.$row->file_name.'" target="_blank">'.$row->file_name.'</a>';
               }
                
                return $btn;
            })
        ->rawColumns(['action','protocol'])
        ->make(true);
        }
    }
