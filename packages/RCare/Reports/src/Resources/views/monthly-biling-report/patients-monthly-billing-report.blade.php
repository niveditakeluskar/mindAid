...
@include('Reports::monthly-biling-report.monthly-billing-filter')
 <div class="row mb-4">
...
@section('page-js')
 ...
 var getMonthlyBillingPatientList = function(practicesgrp=null,practice = null,provider=null,modules=null,monthly=null,monthlyto=null,activedeactivestatus=null,callstatus=null) {
    ...
    $.ajax({
        type: 'GET',
        url: "/reports/monthly-billing-report/search/"+practicesgrp+'/'+practice+"/"+provider+"/"+modules+"/"+monthly+"/"+monthlyto+"/"+activedeactivestatus+"/"+callstatus,
        //data: data,
    ...
    var getPatientData=function()
    {
      $.ajaxSetup({
            ...
            $.ajax({
                type: 'POST',
                url: '/reports/monthly-patient-summary',
                //data: data,
                success: function (data) {

                  var Totalpatient = data.Totalpatient[0]['count'];  
                  var TotalbillablePatient=data.TotalbillablePatient[0]['count'];
                  var EnrolledinCCM = data.TotalEnrolledActiveCcm[0]['count'];
                  var TotalbillablePatientRPM = data.TotalBillableRpm[0]['count'];
                  var EnrolledinRPM = data.TotalEnrolledActiveRPM[0]['count'];
                  var TotalNonbillablePatient=Totalpatient-TotalbillablePatient;
             
                     $('#totalpatient').html(Totalpatient); 
                       $('#billablepatient').html(TotalbillablePatient+'/'+EnrolledinCCM);      
                          $('#nonbillablepatient').html(TotalNonbillablePatient+'/'+EnrolledinCCM); 
                           $('#billablepatientrpm').html(TotalbillablePatientRPM+'/'+EnrolledinRPM);
                            $('#nonbillablepatientrpm').html(TotalbillablePatientRPM+'/'+EnrolledinRPM);    
