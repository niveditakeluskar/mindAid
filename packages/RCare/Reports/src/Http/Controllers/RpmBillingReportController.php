...
{
  public function RpmBillingReportList(Request $request)
  {
    return Inertia::render('Report/BillingReport/RpmBillingReport');
  }
  public function RpmBillingReportSearch(Request $request)
  {
    ...
    for ($i = 0; $i < count($data); $i++) {
      ...
      $arrydata = array(
        $provider_name,
        $practiceemr,
        ucwords(strtolower($data[$i]->pfname)),
        ucwords(strtolower($data[$i]->plname)),
        $data[$i]->pfin_number,
        $pdob,
        $count_distinct_days,
        $reading_days,
