    ...
    //    dd($request->all());
        ...
        ->addIndexColumn()
            ...
            ->addColumn('action', function($row) use ($monthly){ 
                $btn='';
                $cm='';
                $cm .= "<button type='button' data-id='".$row->pid."'   id='shippingdetail' class='btn btn-primary mt-4 shippingdetail' onclick ='shippingdetail(".$row->pid.")'>Shipping Details</button> |<a href='/rpm/timeline-daily-activity/".$row->pid."/timelinedailyactivity' target='_blank' data-id='".$row->pid."' id='timeline' class='btn btn-primary' type='button'>RPM Timeline</a>";
                $btn .= $cm;   
                return  $btn;
            })
