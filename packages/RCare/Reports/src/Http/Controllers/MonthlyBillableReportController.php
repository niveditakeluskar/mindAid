...
class MonthlyBillableReportController extends Controller
{
  ...
  public function MonthlyBillingReportList(Request $request)
  {
    return Inertia::render('Report/BillingReport/MonthlyBillingReport');
  }
  ...
  public function MonthlyBilllingReportPatientsSearch(Request $request)
  {
    ...
    $query = "select * from patients.SP_MONTHLY_BILLING_REPORT($practices,$provider,$module_id,timestamp '" . $dt1 . "',timestamp '" . $dt2 . "',$practicesgrp,'" . $fromdate . "','" . $todate . "',$activedeactivestatus,$callstatus) sp
               left join (select distinct patient_id,count(*)as total,count(quali) as qualified,
