...
class TotalTimeLogReportController extends Controller  
{ 

    public function PatientTimeLogReport(Request $request){
      $configTZ     = config('app.timezone');
      $userTZ       = Session::get('timezone') ? Session::get('timezone') : config('app.timezone');
      $caremanagerid  = sanitizeVariable($request->route('caremanager'));
      ...
      //  $query = "select * from patients.patient_total_time_logs_report('null','null','null','null','null','null',
      //  timestamp '".$dt1."',timestamp '".$dt2."',
      //  '".$configTZ ."','".$userTZ."',
      //  'null')"; 
      $query = "select * from patients.patient_total_time_logs_report($patient,$practiceid,$caremanagerid,$module,$sub_module,$emr,
      timestamp '".$dt1."',timestamp '".$dt2."',
     '".$configTZ ."','".$userTZ."',
      $activedeactivestatus)"; 
        return Inertia::render('Report/TimeLog/TimeLogReport');
    }

    public function totaltimeLogsReportSearch (Request $request){
      $configTZ     = config('app.timezone');
      $userTZ       = Session::get('timezone') ? Session::get('timezone') : config('app.timezone');
      $caremanagerid  = sanitizeVariable($request->route('caremanager'));
      $practiceid = sanitizeVariable($request->route('practiceId')); 
