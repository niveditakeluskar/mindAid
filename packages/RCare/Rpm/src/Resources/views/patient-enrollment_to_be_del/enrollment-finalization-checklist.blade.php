        ...
        <div class="form-group col-md-5" id="sub1">
          <div class="row" >
              ...
              <label class="radio radio-primary col-md-4">
                 <input type="radio" name="Fquestionnaire['{{str_replace(' ','_', trim($value->questionTitle)) }}']" id="Fquestionnaire{{$num}}" value="{{$labels}}"  onchange="chabgeStepSix('<?php echo $labels; ?>','<?php echo 'Fquestionnaire'.$num; ?>');" <?php if($key_exist == 1 && $submited_ans[$nq1] == $labels) { echo "checked";}?> formControlName="radio">
                  <span>{{$labels}}</span>
