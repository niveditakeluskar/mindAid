...
class PrintCarePlanController extends Controller
{
    public function patientCarePlan($subModule = "", $patientId)
    {
        ...
        $patient_vitals =  DB::select("select distinct rec_date as date, height,weight,bmi,bp,o2,pulse_rate,diastolic,oxygen,notes,pain_level
        ...
        foreach ($patient_lab1 as $key => $value) {
            ...
            $patientLabDetails[$date][$lab_test_id]['lab_name'] =  !empty($value['lab_test']['description'])? $value['lab_test']['description']:0;
        ...
        if ($route == 'generate-docx') {
            $html_view = View::make('Ccm::print-care-plan.print-care-plan-pdf', compact(
                'subModule',
                'patient',
            ...
            $pdf  = PDF::loadView('Ccm::print-care-plan.print-care-plan-pdf', compact(
                'subModule',
                'patient',
                'patientDiagnosisDetails',
                'PatientDiagnosis',
