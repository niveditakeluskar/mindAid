...
class CcmController extends Controller { 
    ...
    //     $prev_checklist_topics = DB::select(DB::raw(" ( select schedule_office_appointment,
    //                                 resources_for_medication, medical_renewal, called_office_patientbehalf, referral_support, no_other_services 
    //                                 from ccm.callwrapup_checklist where patient_id = '".$patient_id."'  ) ")       );
    //     //  dd($prev_checklist_topics);

    //     return view('Ccm::previous-month-data', compact('prev_checklist_topics'));
    // }
    ...
    public function patientCarePlanStatus($patient_id,$module_id) {
        ...
        return view('Ccm::print-care-plan.print-care-plan-pdf-body-problem',
                        compact(
                                'subModule', 'patient', 'patientDiagnosisDetails','PatientDiagnosis', 'PatientDiag', 
                                'patient_cmnt', 'patient_providersusers', 'caremanager',
                                ...
                                'patientLabDetails', 'patient_lab1', 'patient_services1', 'patient_healthdata', 'patient_imaging'
                        )
                    );
    }
    ...
    public function fetchMonthlyMonitoringPatientDetails(Request $request) {		
        ...
        $ccmSubModule = $ccmSubModule[0]->id;
    ...
    public function getCallWrapUp($id) {
        ...
        $data  = DB::select(DB::raw( "(select id as \"DT_RowId\", topic, notes, action_taken, status, created_at, id, sequence, sub_sequence 
            ...
            where id in (select max(id)
            FROM ccm.ccm_topics
            WHERE patient_id='".$id."' And topic NOT LIKE 'Summary notes added on%' And topic NOT LIKE 'Additional Services%'
         ...
         (select id as \"DT_RowId\", topic, notes, action_taken, status, created_at, id, sequence, sub_sequence 
            from ccm.ccm_topics  WHERE patient_id='".$id."' And topic LIKE 'Summary notes added on%' and status = 1
            AND EXTRACT(Month from record_date) = '".$month."' AND EXTRACT(YEAR from record_date) = '".$year."' and status = 1 order by id desc)"     
         )  );
        return Datatables::of($data)  
            ->addIndexColumn()
            ->addColumn('action', function($row){
    ...
    public function SaveCallWrapUp(CallwrapAddRequest $request) {
        ...
        DB::beginTransaction();
        try {
			$country_timezone=Session::get('timezone');
    ...
    public function getCcmMonthlyReasearchFollowupData($patientId) {
        ...
        if(CallWrap::where('patient_id', $patientId)->whereMonth('created_at', date('m'))->whereYear('created_at', date('Y'))->exists() ) {
            $CallWrap = CallWrap::where('patient_id', $patientId)
            ->where('sequence',5)
            ->whereMonth('created_at', date('m'))
			->where('status',1)
            ->select(DB::raw("emr_monthly_summary, emr_entry_completed,emr_monthly_summary_date"))
            ->whereYear('created_at', date('Y'))->get(); 
            if(isset($CallWrap[0]->emr_monthly_summary)){ 
                $result['callwrapup_form']['static']['emr_monthly_summary'] = $CallWrap;
            }else{
                $result['callwrapup_form']['static']['emr_monthly_summary'] = ' ';
            }
            if(isset($CallWrap[0]->emr_entry_completed)){ 
                $result['callwrapup_form']['static']['emr_entry_completed'] = $CallWrap[0]->emr_entry_completed;
            }else{
                $result['callwrapup_form']['static']['emr_entry_completed'] = ' '; 
            }

            //changes by ashvini 28thjune2022
            if(isset($CallWrap[0]->emr_monthly_summary_date)){ 
                $result['callwrapup_form']['static']['emr_monthly_summary_date'] = $CallWrap[0]->emr_monthly_summary_date;
            }else{
                $result['callwrapup_form']['static']['emr_monthly_summary_date'] = ' '; 
            }

            $callwrapupchecklistdata = CallWrapupChecklist::where('patient_id', $patientId)->latest()->first();
            // dd($callwrapupchecklistdata);
