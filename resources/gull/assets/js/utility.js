...
var updateCMList = function(practiceId, selectElement, selectedPracticeEmployee = null) {
    selectElement.html($("<option>").html("Select Assign Care Manager"));
    if (!practiceId) {
        return;
    } 
    axios({
        method: "GET",
        url: `/ajax/practice/${practiceId}/PracticeEmployee`,
    }).then(function (response) {
        Object.values(response.data).forEach(function(PracticeEmployee) {
            $("<option>").val(PracticeEmployee.id).html(PracticeEmployee.name).appendTo(selectElement);
        });
        if(selectedPracticeEmployee) {
            selectElement.val(selectedPracticeEmployee);
        }
    }).catch(function (error) {
        console.error(error, error.response);
    });
...
var updateReassignedCMList = function(practiceId, selectElement, selectedPracticeEmployee = null) {
    selectElement.html($("<option>").html("Select Reassign Care Manager"));
    if (!practiceId) {
        return;
    } 
    axios({
        method: "GET",
        url: `/ajax/practice/${practiceId}/PracticeEmployeeReassigned`,
    }).then(function (response) {
        Object.values(response.data).forEach(function(PracticeEmployeeReassigned) {
            $("<option>").val(PracticeEmployeeReassigned.id).html(PracticeEmployeeReassigned.name).appendTo(selectElement);
        });
        if(selectedPracticeEmployee) {
            selectElement.val(selectedPracticeEmployee);
        }
...
window.util = {
    ...
    updateCMList       : updateCMList,
    updateReassignedCMList:updateReassignedCMList,
