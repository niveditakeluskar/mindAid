...
var onCaremanagerNotes = function (formObj, fields, response) {
    if (response.status == 200) {
        ...
        setTimeout(function () { $('#call_step_1_id').click(); }, 5000);
...
$(document).on('click', '.activealertpatientstatus', function () {
    ...
    if ($(this).is(":checked")) {
        if (jQuery.inArray(this.id, addressed_arry) != -1) {
            ...
            //  console.log("anand"+JSON.stringify(rowdata));
...
$(document).on('click', '#cal1 .fc-prev-button', function () {
    ...
    var abc = $(this).closest(".cal").find(".fc-center").html();
    // alert(abc);
    if(abc != undefined){ 
...
var reviewStatusChk = function (this_key) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $('html, body').animate({ scrollTop: $('#patient-alert-history-list_' + activedeviceid).offset().top }, 'slow');
...
var init = function () {
    ...
    $('#calender' + deviceids).show();
        ...
        var abc = $("body").find(".fc-center").html();
        // console.log("abc",abc);
        if(abc != undefined){ 
        var substr = abc.replace('<h2>', '').replace('</h2>', '');
        $('#cald-hid').val(substr);
