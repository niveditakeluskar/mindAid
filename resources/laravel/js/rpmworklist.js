...
var renderDataTableExport = function (tabid, url, columnData, assetBaseUrl, copyflag = "0", copyTo = '') {
                        ...
                        if (index > 0) {
                            ...
                            for (c = 0; c < childData.length; c++) {
                                ...
                                childRow = '<row r="' + rowCount +
                                    ...
                                    '"><is><t>' + srno +
                                    '</t></is></c><c t="inlineStr" r="C' + rowCount +
                                    '"><is><t>' + child.pfname+" "+child.plname +
                                    '</t></is></c><c t="inlineStr" r="D' + rowCount +
                                    '"><is><t>' + child.pdob +
                                     '</t></is></c><c t="inlineStr" r="E' + rowCount +
                                    '"><is><t>' + child.practicename +
                                     '</t></is></c><c t="inlineStr" r="F' + rowCount +
                                    '"><is><t>' + child.providername +
                                     '</t></is></c><c t="inlineStr" r="G' + rowCount +
                                    '"><is><t>' + caremanager +
                                    '</t></is></c><c t="inlineStr" r="H' + rowCount +
                                    '"><is><t>' + vital +
 ...
 var getDailyReviewPatientList = function(practicesgrp = null,practices = null,provider = null,patient = null,caremanagerid = null,fromdate = null,todate =null,reviewedstatus = null) {
            var columns = [
                               { data: 'DT_RowIndex', name: 'DT_RowIndex'},
                                ...
                                { data: 'vital_threshold',name: 'vital_threshold',
                                mRender: function(data, type, full, meta){
            ...
            if (full['alert'] == 1) {
                return '<input type="checkbox" id="activealertpatientstatus_' +meta.row+'_'+full['pid']+'" class="patientaddressed" name="patientaddressed"  value="' + full['pid'] + '" ' + check + '  ' + readonly + ' ><label style="display:none">' + addressstatus + '</label>';
                        ...
                        if(reviewedstatus=='' || reviewedstatus==null){ reviewedstatus=0; } 
                        
            
               
                var url ="/rpm/daily-review-list/search/"+practicesgrp+"/"+practices+"/"+provider+"/"+patient+"/"+caremanagerid+"/"+fromdate+"/"+todate+"/"+reviewedstatus;  
                // console.log(url);
                var parenttable = renderDataTableExport('dailyreviewlist', url, columns, baseURL); 
       ...
       if('indeterminate' in chkbox_select_all){
          chkbox_select_all.indeterminate = true;
                ...
                $('#example-select-all').on('click', function(){   
                    ...
                    var $table = parenttable.table().node();
                    var rows = parenttable.rows({ page: 'current' }).nodes();    
                ...
                $('#example-select-all').click(function(){  
                    ...
                    // var rowcollection =  parenttable.$("input[type='checkbox']:checked",{"page": "all"});
                    // var rowcollection1 =  parenttable.$("input:checkbox:not(:checked)",{"page": "all"});

                    var rowcollection =  parenttable.$("input[type='checkbox']:checked",{"page": "current"});
                    var rowcollection1 =  parenttable.$("input:checkbox:not(:checked)",{"page": "current"});

                    // console.log(rowcollection);
                    rowcollection.each(function(index,elem){
                        // console.log(elem);
                        var row = $(this);
                        var rowdata = parenttable.row($(this).parents('tr')).data();
                        // console.log(rowdata);
        ...
        var getReviewDailyChildList = function(row=null,$Find1=null,patient_id=null,fromdate=null,unittable=null,reviewedstatus=null,serialid=null)
        {
           var columns =  [   
                        {data: 'DT_RowIndex', name: 'DT_RowIndex'},
                        // { data: 'pfname',name: 'pfname',
                    ...
                    { data: 'pfname',name: 'pfname',
                        ...
                        { data: 'vitalthreshold',name: 'vitalthreshold',
                        mRender: function(data, type, full, meta){
            ...
            if($(this).is(":checked")){
                ...
                if(this.id.split("_")[0] == "activealertpatientstatus"){           
                    ...
                    var rowdata = table.row($(this).parents('tr')).data();
                        ...
                        var patient_id = this.id.split("_")[2];
                        // Reviewdaily-Child-list292633552
...
var init = function () { 
   ...
   getDailyReviewPatientList(null,null,null,null,null,fromdate,todate,reviewedstatus);
    ...
    $('#dailyreviewlist tbody').on('click', 'td a', function () {
        ...
        $(this).find('.plus-icons').addClass('i-Add');
