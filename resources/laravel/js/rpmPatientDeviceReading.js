...
var onRPMCMNotesMainForm = function (formObj, fields, response) {
    if (response.status == 200 && $.trim(response.data) == '') {
        ...
        var formname = "form[name='rpm_cm_form']";
        // util.updateTimer($("#p_id").val(), 1, $(formname +" input[name='module_id']").val());
...
var onCareNote = function (formObj, fields, response) {
    if (response.status == 200) {
        var activedeviceid = $('#activedevice').val();
...
var singlereview = function (formnames, data, deviceid, childrowdata, childreviewstatus) {
    $("#time-container").val(AppStopwatch.pauseClock);
    ...
    } else /*if(formname=="childReviewstatus")*/ {
        var activedeviceid = $('#activedevice').val();
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (formnames == "checkreviewclick") {
                ...
                $('html, body').animate({ scrollTop: $('#review-table_' + activedeviceid).offset().top }, 'slow');
...
var checkreviewclick = function (data, deviceid) {
...
var getMonthlyReviewPatientList = function (patientid, deviceid, monthly = null, monthlyto = null) {
    var columns = [{
        data: 'DT_RowIndex',
        name: 'DT_RowIndex'
    },
...
var myactivedeviceid = $('#activedevice').val();

//* =====================for child table ======================================    
var getReviewMonthlyChildList = function (row = null, $Find1 = null, patient_id = null, fromdate = null, unittable = null, reviewedstatus = null, serialid = null) {
    var columns = [{
        data: 'DT_RowIndex',
        name: 'DT_RowIndex'
    },
...
var showtable = function (deviceid) {
    var patient_id = $("#patient_id").val();
    ...
    getMonthlyReviewPatientList(patient_id, deviceid);

    // this is for scroll down 
    $('html, body').animate({ scrollTop: $('#review-table_' + deviceid).offset().top }, 'slow');
}


var dailyReadingFunction = function (patient_id, activateddeviceid) {
...
var reviewStatusChk = function (this_key) {
    ...
    if (reviewstatus == 1 && childdatacount > 0) {
        ...
        //==this is for delete child table first then append table
...
var init = function () {
    ...
    $('#activedevice').val(deviceids);
   
   
    //$("form[name='rpm_text_form_" + deviceids + "'] #text_template_id option:last").attr("selected", "selected").change();
    ...
    $('#monthlyreviewlist_' + activedeviceid + ' tbody').on('click', 'td a', function () {
        ...
        $('#ReviewMonthly-Child-list' + patient_id + '_' + activedeviceid + ' tbody').on('change', '.childreviewpatientstatus', function () {
            ...
            var childrowdata = table.row($(this).parents('tr')).data();

            //  console.log(JSON.stringify(childrowdata));
    ...
    $('.textbtn').click(function () {
        ...
        $('#textcard_' + deviceid[1]).show();
        //alert(deviceid[1]);
        $("form[name='rpm_text_form_" + deviceid[1] + "'] #text_template_id option:last").attr("selected", "selected").change();
        // util.getCallScriptsById($("form[name='rpm_text_form_" + deviceid[1] + "'] #text_template_id option:selected").val(), "form[name='rpm_text_form_" + deviceid[1] + "'] #templatearea_sms", "form[name='rpm_text_form_" + deviceid[1] + "'] input[name='template_type_id']", "form[name='rpm_text_form_" + deviceid[1] + "'] input[name='content_title']");
        // this is for scroll down 
        $('html, body').animate({ scrollTop: $('#textcard_' + deviceid[1]).offset().top }, 'slow');
    ...
    $('#rpm_cm_form .modalcancel,.close').click(function () {
        ...
        for (let j = 0; j < hd_chk_this.length; j++) {
...
$('.tabclass').on('click', function () {
    ...
    //$("form[name='rpm_text_form_" + activedeviceid + "'] #text_template_id option:last").attr("selected", "selected").change();
