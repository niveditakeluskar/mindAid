...
var ajaxSubmit = function (formName, onResult, onErrors) {
    ...
    axios({
        ...
        setTimeout(function () { form.find(":submit").attr("disabled", false) }, 3000);
...
var ajaxForm = function (name, onResult, onSubmit, onErrors) {
	...
	$(document).on('submit','form[name="'+name+'"]',function(){
        //alert("form ID" + $('form[name="' + name + '"]').find(":submit").attr("id"));
        $('form[name="' + name + '"]').find(":submit").attr("disabled", true);
