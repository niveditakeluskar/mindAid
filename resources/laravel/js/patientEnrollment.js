...
var onSaveCallSatus = function (formObj, fields, response) {
    if (response.status == 200) {
        ...
        var scrollPos = $(".main-content").offset().top;
        $(window).scrollTop(scrollPos);
        //  var text = '<button type="button" class="close" data-dismiss="alert">x</button><strong>Data saved successfully! </strong><span id="text"></span>';
        ...
        if ((call_status == '1') && (call_continue_status == '1')) {
            setTimeout(function () { goToNextStep("s2"); }, 3000); //comnt by me
...
var onSaveCallSatusFinal = function (formObj, fields, response) {
...
var onSaveChecklist = function (formObj, fields, response) {
    ...
    if (response.status == 200) {
        ...
        if ($("input[name='patient_reviews_and_signs']:checked").val() == 1) {
            setTimeout(function () { goToNextStep("s3"); $("form[name='patient_reviews_and_signs'] .alert").hide(); }, 3000);
        } else {
            setTimeout(function () { goToNextStep("s4"); $("form[name='patient_reviews_and_signs'] .alert").hide(); }, 3000);
        }
        ...
        var txt = '<div class="alert alert-danger alert-block " style="margin-left: 1.1em;margin-right: 1.1em;"><button type="button" class="close" data-dismiss="alert">× </button><strong>Please Fill All Mandatory Fields!</strong></div>';
        $("#error_msg1").html(txt);
        // if(response.data.errors.enroll_status){
...
var onSaveFinalizationChecklistStatus = function (formObj, fields, response) {
    if (response.status == 200) {
        ...
        $("#error_msg2").html('');
        $('#final_hidden').val(response.data);
        // var text = '<button type="button" class="close" data-dismiss="alert">x</button><strong> Patient enrolled successfully! </strong><span id="text"></span>';
...
var onSaveConcentData = function (formObj, fields, response) {
    if (response.status == 200) {
        $("#concent-alert").show();
        setTimeout(function () { $("#concent-alert").hide(); $('#concent-form').modal('hide'); }, 3000);
    }
};
...
var onMasterDevices = function (formObj, fields, response) {
    if (response.status == 200) {
...
var onPatientThreshold = function (formObj, fields, response) {
    if (response.status == 200) {
        if (sub_module == 'care-plan-development') {
            util.updateTimer($("input[name='patient_id']").val(), $("input[name='billable']").val(), $("input[name='module_id']").val());
        } else {
...
var onAdditionalDeviceEmail = function (formObj, fields, response) {
    if (response.status == 200) {
        if (sub_module == 'care-plan-development') {
            util.updateTimer($("input[name='patient_id']").val(), $("input[name='billable']").val(), $("input[name='module_id']").val());
        } else {
        ...
        //$("form[name='patient_add_device_form'] #email_title option:first").attr("selected", "selected").change();
        $("form[name='patient_add_device_form'] #email_title option:last").attr("selected", "selected").change();
        // $("#add_replace_device").reset();
        setTimeout(function () {
            $("#device-alert").hide();
        }, 3000);
    }
...
var init = function () {
    ...
    $("form[name='text_form']").submit(function (e) {
        ...
        $("#text_form input[name='end_time']").val(timer_paused);
        // $("#timer_start").val(timer_paused);
        $("#timer_end").val(timer_paused);
    ...
    $("form[name='email_form']").submit(function (e) {
        ...
        $("#email_form input[name='end_time']").val(timer_paused);
        // $("#timer_start").val(timer_paused);
        $("#timer_end").val(timer_paused);
        $("#time-container").val(AppStopwatch.startClock);
    ...
    $("form[name='call_status_form']").submit(function (e) {
        ...
        $("#call_status_form input[name='end_time']").val(timer_paused);
        // $("#timer_start").val(timer_paused);
        $("#timer_end").val(timer_paused);
        $("#time-container").val(AppStopwatch.startClock);
    ...
    $("form[name='call_status_form_final']").submit(function (e) {
        ...
        $("#call_status_form_final input[name='end_time']").val(timer_paused);
        // $("#timer_start").val(timer_paused);
        $("#timer_end").val(timer_paused);
        $("#time-container").val(AppStopwatch.startClock);
    ...
    $("form[name='enrollment_status_form']").submit(function (e) {
        ...
        $("#enrollment_status_form input[name='end_time']").val(timer_paused);
        // $("#timer_start").val(timer_paused);
        $("#timer_end").val(timer_paused);
        $("#time-container").val(AppStopwatch.startClock);
    ...
    $("form[name='enroll_services_form']").submit(function (e) {
        ...
        $("#enroll_services_form input[name='end_time']").val(timer_paused);
        // $("#timer_start").val(timer_paused);
        $("#timer_end").val(timer_paused);
        $("#time-container").val(AppStopwatch.startClock);
    ...
    $("form[name='checklist_form']").submit(function (e) {
        ...
        $("#checklist_form input[name='end_time']").val(timer_paused);
        // $("#timer_start").val(timer_paused);
        $("#timer_end").val(timer_paused);
        $("#time-container").val(AppStopwatch.startClock);
    ...
    $("form[name='finalization_checklist_form']").submit(function (e) {
        ...
        $("#finalization_checklist_form input[name='end_time']").val(timer_paused);
        // $("#timer_start").val(timer_paused);
        $("#timer_end").val(timer_paused);
        $("#time-container").val(AppStopwatch.startClock);
    ...
    $("form[name='checklist_status_form']").submit(function (e) {
        ...
        $("#checklist_status_form input[name='end_time']").val(timer_paused);
        // $("#timer_start").val(timer_paused);
        $("#timer_end").val(timer_paused);
        $("#time-container").val(AppStopwatch.startClock);
...
$('form[name="patient_add_device_form"] .submit-patient-add_device').on('click', function () {
    ...
    $("form[name='patient_add_device_form'] [name='end_time']").val(timer_paused);
    // $("#timer_start").val(timer_paused);
