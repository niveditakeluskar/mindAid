...
var resetPatientRegisterationForm = function () {
...
$(".confirmation").click(function () {
    ...
    setTimeout(function () {
        ...
        // Redirect on enrollment page  
        if (confirmation != 'No') {
            if (module_id != '') {
                window.location.href = '/patients/patient-enrollment/' + patientId + '/' + module_id;
                // $("form[name='patient_registration_form']")[0].reset();
                resetPatientRegisterationForm();
                $("form[name='patient_registration_form'] #practices").val('');
                $("form[name='patient_registration_form'] #physician").val('');
...
var init = function () {
    ...
    form.ajaxForm("patient_registration_form", onPatientRegisteration, function () {
        var uidError = $('form[name="patient_registration_form"]  #uid').next(".invalid-feedback").html();
            if (uidError.length > 0) {//uidError == "Patient with this First Name, Last Name and DOB already exists.") {
                $('#error_msg').show();
                $('#error_msg1').show();
                ...
                $('#error_msg').html(txt);
                $('#error_msg1').html(txt);
                // $("form[name='patient_registration_form'] input[name='lname']").focus();
                ...
                $("input[name='timer_end']").val(timer_paused);
    ...
    */
    ...
    $("#practices").on("change", function () {
        ...
        $(".providers_name").val('');
        $(".providers_name").hide();
        //util.updatePcpPhysicianList(parseInt($(this).val()), $("#physician"));
        var id = $(this).val();
        $.ajax({
