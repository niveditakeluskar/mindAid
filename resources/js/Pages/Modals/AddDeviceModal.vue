      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          ...
          <form
            ...
            @submit.prevent="submitadddeviceForm"
          >
            <input
              type="hidden"
              name="patient_id"
            ...
            >
              ...
              <strong>Additional Device data saved successfully! </strong
              ><span id="text"></span>
            </div>
            <div class="row">
              <div class="col-md-12 form-group mb-3">
                ...
                <select
                  name="add_replace_device"
                  id="add_replace_device"
                  class="custom-select show-tick select2"
                  v-model="addReplaceDevice"
                ...
                <div
                  ...
                  v-if="formErrors.add_replace_device"
                  style="display: block"
                >
                  {{ formErrors.add_replace_device[0] }}
                </div>
              </div>
              <div class="col-md-6 form-group mb-3">
                ...
                <div class="wrapMulDropDevices">
                  ...
                  <ul ref="deviceList" style="list-style-type: none"></ul>
                </div>
              </div>
              <div class="col-md-6 form-group mb-3">
                ...
                <select
                  ...
                  name="call_not_answer_template_id"
                  v-model="selectedEmail"
                  @change="textScript(selectedEmail)"
                  required
                >
                ...
                <div
                  class="invalid-feedback"
                  v-if="formErrors.call_not_answer_template_id"
                  style="display: block"
                >
                  {{ formErrors.call_not_answer_template_id[0] }}
                </div>
              </div>
              ...
              <div class="col-md-12 form-group mb-3">
                ...
                <textarea
                  ...
                  v-model="editorData"
                  id="email_title_area"
                  style="padding: 5px;width: 47em;min-height: 5em;overflow: auto;height: 87px;}"
                ></textarea>
                <div
                  class="invalid-feedback"
                  v-if="formErrors.text_msg"
                  style="display: block"
                >
                  {{ formErrors.text_msg[0] }}
                </div>
              </div>
            </div>
            <div class="modal-footer">
              <button
                type="submit"
                class="btn btn-primary float-right submit-patient-add_device"
              >
                Submit
              </button>
              <button
  ...
  setup(props) {
    ...
    const submitadddeviceForm = async () => {
      ...
      try {
        ...
        if (error.response && error.response.status === 422) {
          ...
          alert(
            "oopz! something went wrong. Try again or Contact Administrator"
          );
    ...
    let fetchEmail = async () => {
      try {
        ...
        const emailArray = Object.entries(emailData).map(
          ([id, content_title]) => ({ id, content_title })
        );
        if (emailArray.length > 0) {
          // Check if emailArray is not empty
    ...
    let textScript = async (id) => {
      await axios
        .get(`/ccm/get-call-scripts-by-id/${id}/${props.patientId}/call-script`)
        .then((response) => {
    ...
    const getDevice = (checkbox) => {
      if (editorData.value && checkbox.checked) {
        ...
        const text = editorData.value
          .getData()
          .replace(`<li>${checkbox.id}</li>`, "")
          .trim();
    ...
    let getStageID = async () => {
      try {
        ...
        let response = await axios.get(
          `/get_stage_id/${props.moduleId}/${props.componentId}/${templateName}`
        );
