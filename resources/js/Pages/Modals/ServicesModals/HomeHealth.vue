    ...
    <div class="tab-pane fade show active" id="home-health-services" role="tabpanel" aria-labelledby="home-health-services-icon-pill">
        <div class="card">  
            ...
            <form id="service_home_health_form" name="service_home_health_form" @submit.prevent="submitSrvicesForm">
                <div class="alert alert-success" :style="{ display: showHomeHealthAlert ? 'block' : 'none' }">
                    <button type="button" class="close" data-dismiss="alert">x</button>
                    <strong> Home Health Services data saved successfully! </strong><span id="text"></span>
                </div>  
                <div class="form-row col-md-12">
                    ...
                    <input type="hidden" name="stage_id" :value="homeHealthServicesStageId"/>
                    <input type="hidden" name="step_id" :value="homeHealthServicesStepId">
                    <input type="hidden" name="form_name" value="service_home_health_form">
                    <input type="hidden" name="service_type" value="home_health_services">
        		    <input type="hidden" name="hid" class="hid" value='2'>
                    <input type="hidden" name="id" id="service_id">
                    <input type="hidden" name="billable" value="1">
                    <input type="hidden" name="timearr[form_start_time]" class="timearr form_start_time" :value="HomeHealthServicesTime" />
                    <HomeHealthForm :formErrors="formErrors" />
                </div>
                <div class="card-footer">
                    <div class="mc-footer">
                        <div class="row">
                            <div class="col-lg-12 text-right">
                                <button type="submit" class="btn  btn-primary m-1" id="save_service_home_health_form">Save</button>
                            </div>
                        </div>
                    </div>
                </div>
...
export default {
    ...
    setup(props) {
        ...
        let homeHealthServicesStageId = ref(0);
        let homeHealthServicesStepId = ref(0);
        ...
        const fetchPatientHomeHealthServiceList = async () => {
            try {
                ...
                const response = await fetch(`/ccm/care-plan-development-services-list/${props.patientId}/2?mm=${mm}`);
                if (!response.ok) {
                    throw new Error('Failed to fetch followup task list');
                }
                loading.value = false;
                const data = await response.json();
                homeHealthServiceRowData.value = data.data;
            } catch (error) {
        ...
        let submitSrvicesForm = async () => {
            ...
            try {
                const saveServicesResponse = await saveServices(formDataObject);
                    ...
                    document.getElementById("service_home_health_form").reset();
                    setTimeout(() => {
                        showHomeHealthAlert.value = false;
        ...
        let deleteServices = async (id, obj) => {
            if (window.confirm("Are you sure you want to delete this Service?")) {
                const formData = {
                    ...
                    step_id: homeHealthServicesStepId.value,
                    form_name: 'service_home_health_form',
                    billable: 1,
                    start_time: "00:00:00",
                    end_time: "00:00:00",
        ...
        let editService = async (id) => {
            try {
                const serviceToEdit = homeHealthServiceRowData.value.find(service => service.id == id);
                if (serviceToEdit) {
                    const form = document.getElementById('service_home_health_form');
        ...
        watch(() => showHomeHealthAlert, (newShowHomeHealthAlert, oldShowHomeHealthAlert) => {
                showHomeHealthAlert.value = newShowHomeHealthAlert;
            }
        );
        ...
        return {
            loading,
            submitSrvicesForm,
            homeHealthServicesStageId,
            homeHealthServicesStepId,
            formErrors,
            HomeHealthServicesTime,
            showHomeHealthAlert,
            columnDefs,
