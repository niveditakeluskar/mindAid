              ...
              <div class="form-row">
                ...
                <div class="col-md-2 form-group mt-4 ml-2">
                  <label for="only_code" class="checkbox checkbox-primary mr-3">
                    <input type="checkbox" name="only_code" id="only_code" class="CRclass" formcontrolname="checkbox"
                      value="1" v-model="onlycode">
...
export default {
  ...
  setup(props) {
    ...
    let columnDefs = ref([
      ...
      { headerName: 'Vendor', field: 'partner_name' },
      ...
      {
        headerName: 'Enrolled Services', field: 'enrolled_modules',
        cellRenderer: function (params) {
          let enrolled_modules = params.data.enrolled_modules;
          if (enrolled_modules == null || enrolled_modules == 'null') {
            enrolled_modules = '';
          }
          if (enrolled_modules == '2,3') {
            enrolled_modules = 'RPM,CCM';
          }
          if (enrolled_modules == '3,2') {
            enrolled_modules = 'CCM,RPM';
          }
          if (enrolled_modules == '3') {
            enrolled_modules = 'CCM';
          }
          if (enrolled_modules == '2') {
      ...
      { headerName: 'Number of Days Readings', field: 'count_distinct_days' },
      { headerName: 'Day of Month of Days Readings', field: 'reading_days' },
      { headerName: 'Number of Unique Readings', field: 'total_reading_days' },
      {
        headerName: 'RPM(DOS)', field: 'dos',
        cellRenderer: function (params) {
      ...
      { headerName: 'Billing Threshold # Days', field: 'billing_threshold_days' },
      { headerName: 'Billing Threshold Met', field: 'reading_exceeds' },
      {
        headerName: 'Device Education Date', field: 'dtrec_date',
        cellRenderer: function (params) {
          const date = params.data.dtrec_date;
          if (!date) return null;
          const formattedDate = new Date(date).toLocaleDateString('en-US', {
            year: 'numeric',
            month: '2-digit',
      ...
      { headerName: 'Qualifying Conditions', field: 'qualified' },
    ...
    const fetchFilters = async () => {
      try {
        ...
        const response = await fetch('/reports/consolidate-searh-data/' + selectedGroupids + '/' + selectedPractices + '/' + selectedProviders + '/' + m + '/' + activedeactivestatuse + '/' + callstatuse + '/' + onlycodes + '/search');
        ...
        console.log(passRowData.value[7]);
        // if(passRowData.value.pdcode != null && passRowData.value.pdcode != 'null' && passRowData.value.pdcode != '' && passRowData.value.pdcode != undefined){
        //   passRowData.value.pdcode.replace("|", ",");
        //   console.log(passRowData.value.pdcode);
        // }
