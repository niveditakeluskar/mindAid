...
const title = "RPM Billing Report";
...
const fetchPatients = async () => {
  try {
    const response = await fetch(
      "/patients/ajax/practice/" + filterState.practiceId + "/2/patient"
    );
    if (!response.ok) {
...
const fetchClinicalInsightReportApiData = async () => {
  try {
    ...
    const response = await fetch(
      `/reports/rpm-billing-report/${filterState.patientId}/${filterState.practiceId}/${filterState.fromDate}/${filterState.toDate}/2/${filterState.provider}/${filterState.organization}/${filterState.status}/${filterState.callAnsweredStatus}`,
      {
        method: "GET",
        headers: {
